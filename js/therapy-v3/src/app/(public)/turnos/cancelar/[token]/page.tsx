...
export default function CancelarPage() {
  ...
  useEffect(() => {
    async function cancel() {
      try {
        const result = await appointmentsApi.cancel(token);
        setStatus("success");
  ...
  return (
    <div className="max-w-[720px] mx-auto px-5 py-16 text-center">
      ...
      {status === "success" && (
        <>
          <div className="inline-flex w-[84px] h-[84px] rounded-full bg-[var(--color-warning-bg)] border border-[rgba(176,124,42,0.2)] text-[var(--color-warning)] items-center justify-center mb-6">
